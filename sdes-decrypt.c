...
void decrypt_file(char *encrypted_filename, char *output_filename, uint16_t key, uint8_t num_rounds) {
	...
	while (!feof(input_file)) {
		...
		if (num_read == 3) {
			...
			uint16_t block1 = input_val & 0xFFF;
			uint16_t block2 = input_val >> 12;

			uint16_t decrypted_block2 = sdes_decrypt(block2, keys, num_rounds);
			uint16_t decrypted_block1 = sdes_decrypt(block1, keys, num_rounds);

			uint32_t decrypted_val = decrypted_block1 | (decrypted_block2 << 12);

			if (remaining_reads == 0) {
				fwrite(&decrypted_val, 1, (3 - padding), output_file);
			}
			else {
				fwrite(&decrypted_val, 1, 3, output_file);
			}
		}
